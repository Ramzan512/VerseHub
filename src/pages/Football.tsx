         ...
         {TABS.map(tab => {
            ...
            return (
               <button
                 ...
                 className={`flex items-center gap-2 px-5 py-3 rounded-xl font-bold whitespace-nowrap transition-all duration-300 ${isActive ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30 shadow-[0_0_20px_rgba(59,130,246,0.2)]' : 'bg-black/40 text-muted-foreground border border-white/5 hover:bg-white/5'}`}
               >
...
function MatchCenter({ data }: { data: any }) {
  if (!data?.events) {
    return (
      <div className="h-64 flex items-center justify-center border border-white/10 bg-black/40 rounded-2xl w-full">
  ...
  return (
    <div className="flex flex-col gap-6">
       <div className="flex items-center justify-between">
          ...
          <Button variant="outline" className="text-xs bg-black/40 border-white/10 hover:bg-white/5">
          ...
          {events.map((event: any) => {
             ...
             return (
               <Card key={event.id} className="bg-[#0b1426]/80 border-blue-500/20 overflow-hidden hover:border-blue-500/40 hover:shadow-[0_0_30px_rgba(59,130,246,0.15)] transition-all cursor-pointer group">
                  <div className="bg-black/40 px-4 py-2 text-xs font-bold tracking-wider flex justify-between uppercase border-b border-white/5">
                  ...
                  <CardContent className="p-6">
                     <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-col items-center gap-3 w-1/3">
                           <div className="w-16 h-16 rounded-full bg-white/5 p-2 flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform">
                              <img src={home?.team?.logo || 'https://via.placeholder.com/64'} alt={home?.team?.name} className="w-12 h-12 object-contain" />
                           </div>
                           <span className="font-bold text-white text-center text-sm md:text-base leading-tight">{home?.team?.shortDisplayName || home?.team?.name}</span>
                        </div>
                        
                        {/* Score */}
                        <div className="flex flex-col items-center justify-center w-1/3">
                           <div className="flex items-center gap-4 text-3xl md:text-4xl font-black">
                              <span className={matchStatus === 'pre' ? 'text-white/40' : 'text-white'}>{home?.score ?? '-'}</span>
                              <span className="text-white/20 text-lg">vs</span>
                              <span className={matchStatus === 'pre' ? 'text-white/40' : 'text-white'}>{away?.score ?? '-'}</span>
                           </div>
                        ...
                        <div className="flex flex-col items-center gap-3 w-1/3">
                           ...
                           <span className="font-bold text-white text-center text-sm md:text-base leading-tight">{away?.team?.shortDisplayName || away?.team?.name}</span>
                        </div>
                     </div>
                  </CardContent>
               </Card>
    ...
    <div className="flex flex-col gap-8">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {groups.map((group: any) => {
             ...
             return (
               <div key={group.id} className="bg-[#0b1426]/80 rounded-2xl border border-white/10 overflow-hidden">
                  <div className="bg-black/60 px-4 py-3 font-bold text-blue-400 border-b border-white/10 flex justify-between items-center">
                     {group.name}
                     <span className="text-xs font-normal text-white/50 uppercase">Pt | W-D-L (GD)</span>
                  </div>
                  <div className="flex flex-col">
                     {standings.map((entry: any, i: number) => {
                        return (
                           <div key={entry.team.id} className={`flex items-center justify-between p-3 px-4 ${i !== standings.length-1 ? 'border-b border-white/5' : ''} hover:bg-white/5 transition-colors`}>
                              <div className="flex items-center gap-3">
                                 <span className="font-mono text-white/40 w-4 font-bold">{i+1}</span>
                                 <img src={entry.team.logos?.[0]?.href || ''} className="w-6 h-6 object-contain" alt="" />
                                 <span className="font-bold text-white">{entry.team.displayName}</span>
                              </div>
                              <div className="flex items-center gap-4 text-sm font-mono">
                                 <span className="text-white font-bold">{entry.stats.find((s:any)=>s.name==='points')?.displayValue}</span>
                                 <div className="flex gap-2 text-white/50 w-24 justify-end">
                                    ...
                                    <span className="w-8 text-right font-bold text-white/70">({entry.stats.find((s:any)=>s.name==='pointDifferential')?.displayValue})</span>
                                 </div>
                              </div>
                           </div>
                        )
...
function TeamsSearch({ query, setQuery }: { query: string, setQuery: (q:string) => void }) {
  ...
  if (selectedTeam) {
    ...
    return (
       <div className="flex flex-col gap-6 w-full relative z-10">
          {/* Header */}
          <div className="flex flex-col md:flex-row bg-[#0b1426]/80 p-6 rounded-2xl border border-white/10 items-start md:items-center justify-between gap-4">
             <div className="flex items-center gap-4">
                 ...
                 <div>
                    <h2 className="text-2xl md:text-3xl font-bold text-white leading-tight">{selectedTeam.team.displayName} Squad</h2>
                    <p className="text-white/50 text-sm mt-1">FIFA World Cup 2026 Roster {selectedTeam.team.standingSummary && `• ${selectedTeam.team.standingSummary}`}</p>
                 </div>
             </div>
             <div className="hidden md:flex flex-col items-end gap-1">
                {selectedTeam.team.location && <span className="text-white/50 font-medium text-sm flex items-center gap-2"><MapPin className="w-3.5 h-3.5"/>{selectedTeam.team.location}</span>}
          ...
          {/* Roster Area */}
          {rosterLoading ? (
            <div className="h-64 border border-white/10 bg-[#0b1426]/60 rounded-2xl flex flex-col items-center justify-center shadow-inner">
  ...
  return (
    <div className="flex flex-col gap-6">
       <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#0b1426]/80 p-6 rounded-2xl border border-white/10">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          <div className="relative w-full sm:w-auto">
             ...
             <input 
                ...
                className="w-full sm:w-80 bg-black/60 border border-white/10 rounded-full py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-blue-500/50"
             />
          </div>
       </div>
       ...
       {teamsData ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
             {filtered.map((t: any) => (
                <Card key={t.team.id} onClick={() => fetchRoster(t)} className="bg-[#0b1426]/60 border-white/10 hover:border-blue-500/40 hover:bg-blue-500/5 transition-all cursor-pointer group">
                   <CardContent className="p-6 flex flex-col items-center gap-4">
                      <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center p-3 border border-white/10 group-hover:scale-110 transition-transform">
                         <img src={t.team.logos?.[0]?.href} className="w-full h-full object-contain" alt={t.team.name} />
                      </div>
                      <div className="text-center">
                         <h3 className="font-bold text-white text-lg">{t.team.displayName}</h3>
                         <p className="text-xs text-white/50">{t.team.abbreviation}</p>
                      </div>
                      <Button variant="outline" className="w-full text-xs bg-black/40 border-white/10 group-hover:bg-blue-600 group-hover:border-blue-600 transition-colors pointer-events-none">
                         View Squad
                      </Button>
                   </CardContent>
                </Card>
...
function RosterGroup({ title, players }: { title: string, players: any[] }) {
   ...
   return (
      <div className="flex flex-col gap-4">
         <h3 className="text-xl font-bold text-white border-b border-white/10 pb-2">{title}</h3>
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {players.map((p: any) => (
               <Card key={p.id} className="bg-[#0b1426]/60 border-white/5 overflow-hidden hover:border-blue-500/30 hover:bg-[#0f1d35] transition-colors group">
                  <CardContent className="p-4 flex gap-4 items-center">
                     <div className="relative w-14 h-14 bg-black/40 rounded-full overflow-hidden border border-white/10 shrink-0 shadow-inner group-hover:border-blue-500/50">
                        {p.headshot?.href ? (
                        ...
                        {p.jersey && (
                           <div className="absolute -bottom-1 -right-1 bg-blue-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full border border-[#0b1426]">
                              #{p.jersey}
                           </div>
                        )}
                     </div>
                     <div className="flex flex-col min-w-0 flex-1">
                        <h4 className="text-white font-bold truncate text-sm" title={p.displayName}>{p.displayName}</h4>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                           <span className="text-[10px] text-white/50">{p.age ? `${p.age} yrs` : ''}</span>
                           {p.citizenship && (
                              <>
                                 <span className="text-white/20 text-[10px]">•</span>
                                 <span className="text-[10px] text-white/50 truncate w-full">{p.citizenship}</span>
                              </>
                           )}
                        </div>
                     </div>
      ...
      <div className="flex flex-col gap-6">
         <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {stadiums.map((s, idx) => (
                <Card key={idx} className="bg-[#0b1426]/80 overflow-hidden border-white/10 group cursor-pointer">
                   <div className="h-48 overflow-hidden relative">
                      ...
                      <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 text-xs font-bold text-white flex items-center gap-1">
    ...
    <div className="flex flex-col gap-6 w-full max-w-5xl mx-auto">
       <div className="flex flex-wrap items-center gap-2 mb-2">
          {filters.map(f => (
             <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${filter === f ? 'bg-blue-600/30 text-blue-400 border border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.3)]' : 'bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10'}`}
             >
                {f}
             </button>
          ))}
          <div className="relative ml-auto flex-1 min-w-[200px] max-w-sm">
             <input
                ...
                className="w-full bg-black/40 border border-white/10 rounded-full px-4 py-2 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all"
             />
          </div>
       </div>
       ...
       {loading ? (
          <div className="text-center py-12 bg-white/5 rounded-2xl border border-white/5">
             ...
             <p className="text-white/60 font-medium">Loading World Cup 2026 Fixtures...</p>
          </div>
       ) : (
          <>
             <div className="grid grid-cols-1 gap-4">
                {matches.map((match: any) => {
                   ...
                   return (
                      <Card key={match.id} className="bg-gradient-to-r from-black/60 to-black/40 border-white/10 overflow-hidden relative group hover:border-blue-500/30 transition-all duration-300">
                         ...
                         <CardContent className="p-0">
                            <div className="flex flex-col md:flex-row items-stretch">
                               <div className="md:w-48 bg-white/5 p-4 flex flex-row md:flex-col items-center justify-between md:justify-center border-b md:border-b-0 md:border-r border-white/5 gap-2">
                                  <div className="text-center">
                                     <p className="text-white/60 text-xs font-bold uppercase tracking-wider mb-1">{dateString}</p>
                               ...
                               <div className="flex-1 p-5 md:p-6 flex flex-col justify-center">
                                  <div className="flex flex-wrap items-center justify-between mb-4 gap-2">
                                     ...
                                     <span className="flex items-center gap-1.5 text-[10px] md:text-xs text-white/40 font-medium text-right">
                                  ...
                                  <div className="flex items-center justify-between mt-2 gap-4">
                                     <div className="flex-1 flex items-center justify-end gap-3 md:gap-4 group-hover:translate-x-1 transition-transform">
                                        <span className="text-sm sm:text-lg md:text-2xl font-black text-white text-right leading-tight">{homeTeam?.displayName || "TBD"}</span>
                                        {homeTeam?.logo ? (
                                     ...
                                     <div className="px-3 sm:px-4 py-2 bg-black/40 rounded-xl border border-white/5 font-black text-white/40 text-sm sm:text-lg mx-1 sm:mx-2">
                                        VS
                                     </div>
                                     ...
                                     <div className="flex-1 flex items-center justify-start gap-3 md:gap-4 group-hover:-translate-x-1 transition-transform">
                                        ...
                                        <span className="text-sm sm:text-lg md:text-2xl font-black text-white leading-tight">{awayTeam?.displayName || "TBD"}</span>
                                     </div>
                                  </div>
                               </div>
                            </div>
                ...
                {matches.length === 0 && (
                   <div className="text-center py-12 bg-white/5 rounded-2xl border border-white/5 backdrop-blur-sm">
                      ...
                      <p className="text-white/60 font-medium text-lg">No matches found for this filter.</p>
                   </div>
                )}
             </div>
             ...
             {hasMore && (
                <div className="mt-6 flex justify-center">
                   <button 
                      ...
                      className="px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white/80 font-bold hover:bg-white/10 transition-colors duration-300 shadow-[0_0_15px_rgba(0,0,0,0.5)]"
                   >
                      Load More Matches
                   </button>
                </div>
