      ...
      <CardContent className="p-0 flex flex-col h-full bg-gradient-to-br from-[#0F172A]/80 to-[#1E1B4B]/80 rounded-[2rem] border border-[#00BFFF]/40 shadow-[0_0_30px_rgba(0,191,255,0.15)] hover:border-[#00BFFF]/60 hover:shadow-[0_0_50px_rgba(0,191,255,0.3)] transition-all relative overflow-hidden backdrop-blur-xl group">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 sm:p-8 border-b border-[#00BFFF]/20 bg-gradient-to-r from-[#00BFFF]/10 to-[#8A2BE2]/10 z-10 sticky top-0 backdrop-blur-xl">
           <div className="flex items-center gap-4 mb-4 sm:mb-0">
             ...
             <div>
               <h3 className="text-2xl font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 drop-shadow-[0_0_15px_rgba(0,255,255,0.6)] flex items-center gap-2 mb-1">
                 CRYPTO LIVE NEWS <span className="text-[10px] bg-red-500 text-text px-2 py-0.5 rounded-full uppercase tracking-wider font-bold shadow-sm shadow-red-500/50">Live Feed</span>
               </h3>
           ...
           <div className="flex items-center gap-3">
             <div className="text-xs font-semibold text-text-muted bg-card border border-[#00BFFF]/30 px-3 py-1.5 rounded-lg shadow-inner">
                <span className="text-[#00FFFF] font-bold tracking-wide">{news.length}</span> Breaking Updates
             </div>
             {lastUpdated && (
               <div className="text-[10px] text-text-muted hidden md:flex items-center gap-1 font-mono uppercase tracking-wider">
                 <RefreshCw className="w-3 h-3 text-[#00FFFF]/50" /> updated {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
               </div>
             )}
             <button onClick={fetchNews} disabled={loading} className="p-2 bg-card hover:bg-white/10 border border-[#00BFFF]/30 rounded-lg transition-colors cursor-pointer disabled:opacity-50 text-text-muted hover:text-text">
        ...
        <div className="p-4 sm:p-6 flex-1 relative z-0 flex flex-col">
          {loading && news.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center min-h-[300px] text-text-muted space-y-4">
              <div className="relative">
            ...
            <div className="flex-1 flex items-center justify-center min-h-[300px] text-[#00FFFF]/50 text-sm font-medium border border-dashed border-[#00BFFF]/20 rounded-2xl bg-card text-center px-4">
            ...
            <div className="flex flex-col h-full w-full">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 flex-1 w-full">
                {news.filter(item => item && (item.headline || (item as unknown as any).title || item.link || item.source)).slice(0, 3).map((item, idx) => {
                  ...
                  return (
                    ...
                    >
                      {/* Image Banner */}
                      <div className="w-full h-[200px] relative overflow-hidden bg-[#0A1020] border-b border-border shrink-0">
                         <img 
                           src={imageUrl} 
                           alt={item.headline} 
                      ...
                      <div className="flex flex-col flex-1 p-5 relative">
                         ...
                         <div className="mb-3 flex-1 flex flex-col">
                            <h4 className="text-[18px] lg:text-[20px] font-bold text-text tracking-normal leading-snug transition-colors line-clamp-2 mb-3">
                               {item.headline || (item as any).title || (item as any).name || "News title unavailable"}
                            </h4>
                            {/* Summary */}
                            <p className="text-xs sm:text-sm text-text-muted line-clamp-3 leading-relaxed mt-auto">
              ...
              <div className="mt-8 relative z-20">
                <Link to="/news" onClick={() => trackEvent('News Widget Open Hub Clicked')} className="flex items-center justify-center w-full bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 hover:from-cyan-400 hover:via-blue-400 hover:to-purple-500 text-text font-black text-[15px] tracking-widest uppercase py-4 rounded-2xl shadow-[0_0_20px_rgba(0,255,255,0.3)] hover:shadow-[0_0_30px_rgba(0,255,255,0.5)] transition-all transform hover:-translate-y-0.5 relative overflow-hidden group border border-border">
